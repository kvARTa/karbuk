...
<div class="box-content">
 ...
 <div>
       ...
       <?php if ($thumb) { ?>
      <div class="image"><img align="left" src="<?php echo $thumb; ?>" title="<?php echo $heading_title; ?>" alt="<?php echo $heading_title; ?>" id="image" /></div>
      <?php } ?>

<strong>  <?php echo $date_added; ?></strong><br /> <br />
...
<?php
	...
	if($tags){
		...
		foreach($tags as $tag){
			if($i > 43) break;
			$i++;
			$tag = mysql_fetch_assoc(mysql_query("SELECT product_id FROM `product` WHERE model = ".$tag["tag"]));
			$products[] = $this->model_catalog_product->getProduct($tag["product_id"]);
		}
	}
?>
<div class="product-grid blog">
<?php foreach($products as $product){ ?>
	<div class="grey">
		<div style="height:265px;">
		<div class="name"><a href="index.php?route=product/product&product_id=<?=$product["product_id"]?>"><?=$product["name"]?></a></div>
		<div class="image"><a href="index.php?route=product/product&product_id=<?=$product["product_id"]?>">
			<img style="width: auto;height:106px;" src="image/cache/data/<?=$product["model"]?>-184x136.jpg">
		</a></div>
		</div>
		<div class="price">
		...
		<div class="cart">
			<input type="text" value="1" id="item-<?=$product["product_id"]?>" class="input_q">
		...
		<div class="wishlist" id="wishlist<?=$product["product_id"]?>">
			...
			<a class="wish" onclick="addToWishList('<?=$product["product_id"]?>');">
			Добавить в избранные      	</a>
			<a class="textRed gowish" style="display:none" href="index.php?route=account/wishlist">
			Перейти в избранные
			</a>
		</div>
		<div class="compare" id="compare<?=$product["product_id"]?>">
			<input onclick="addToCompare('<?=$product["product_id"]?>');" name="compare" type="checkbox" value="">
			<a class="comp" onclick="addToCompare('<?=$product["product_id"]?>');">
			Добавить для сравнения      	</a>
			<a class="textRed gocomp" style="display:none" href="index.php?route=product/compare" rel="shadowbox; width=1024">
			Перейти к сравнению
			</a>
		</div>
	</div>
