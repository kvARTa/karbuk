...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
		...
		if (!empty($this->request->post['quantity'])) {
			...
			foreach ($this->request->post['quantity'] as $key => $value) {
				...
				if ($product_info['multiplicity'] > 1)
					$this->cart->update($key, (ceil($value / $product_info['multiplicity']))*$product_info['multiplicity']);
				else  
	...
	public function add() {
		...
		if ($product_info) {			
			if (isset($this->request->post['quantity'])) {
				if ($product_info['multiplicity'] > 1)
					$quantity = (ceil($this->request->post['quantity'] / $product_info['multiplicity']))*$product_info['multiplicity'];
				else{
	...
	public function add_model() {
		...
		if (isset($this->request->post['model_to_cart'])){
			$arrModels = explode(",",$this->request->post['model_to_cart']);
			foreach ($arrModels as $model) {
				$product_id = $this->model_catalog_product->getProductid_by_model($model);
				if ($product_id !== null){
					$product_info = $this->model_catalog_product->getProduct($product_id);
					if ($product_info['multiplicity'] == 1)
						$quantity = $product_info['minimum'];
					elseif ($product_info['multiplicity'] > 1){
						$quantity = (floor($product_info['minimum'] / $product_info['multiplicity'])+1)*$product_info['multiplicity'];
					}
    ...
    public function callback() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ((utf8_strlen($this->request->post['call_name']) < 3) || (utf8_strlen($this->request->post['call_name']) > 20)) {
                $json['msg'] = $this->language->get('error_text');
            }

            if ((utf8_strlen($this->request->post['call_phone']) < 5) || (utf8_strlen($this->request->post['call_phone']) > 20)) {
                $json['msg'] = $this->language->get('error_text');
            }

            if ((utf8_strlen($this->request->post['call_company']) < 2) || (utf8_strlen($this->request->post['call_phone']) > 50)) {
                $json['msg'] = $this->language->get('error_text');
            }

            
            if ((utf8_strlen($this->request->post['call_email']) < 2) || (utf8_strlen($this->request->post['call_phone']) > 50)) {
                $json['msg'] = $this->language->get('error_text');
            }
            
            if(!isset($json['msg'])) {
            
                $subject = $this->language->get('text_mail_subj');
                
                $message = sprintf($this->language->get('text_mail_msg'), $this->request->post['call_name'],$this->request->post['call_phone'], $this->request->post['call_company'], $this->request->post['call_email'] );
                ...
                $json['msg'] = $this->language->get('text_mail_success');
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($_FILES['filecsv'])) {
            ...
            foreach ($csvData as $csvRow){
                ...
                if (trim($csv[0]) != 'Артикул'){
                    //echo $csv[0] .' ' . $csv[1];
                    $this->cart->add(trim($csv[0]), trim($csv[1]));
                }
            }
        }
