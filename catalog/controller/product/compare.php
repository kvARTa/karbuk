...
class ControllerProductCompare extends Controller {
		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
				...
				$this->data['products'][$product_id] = array(
					...
					'multiplicity' => $product_info['multiplicity'],
	...
	public function add() {
		...
		//Proverim, v odnoy li kategorii nahodyatsya tovary dlya sravneniya, esli oni uzhe est
		if (count($this->session->data['compare']) > 0) {
		    //Dostatochno odnogo tovara
		    $query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE 
			product_id = '" . $this->session->data['compare'][0] . "'");
		    $categ1 = $query->rows[0]['category_id'];
		    $query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE 
			product_id = '" . $this->request->post['product_id'] . "'");
		    $categ2 = $query->rows[0]['category_id'];
		    //Esli kategorii ne sovpadayut - sbrasyvaem sravnenie
		    if ($categ1!=$categ2){
			$this->session->data['compare'] = array();
		    }
		...
		if ($product_info) {
			...
			if (!in_array($this->request->post['product_id'], $this->session->data['compare'])) {
				if (count($this->session->data['compare']) >= 44) {
