...
class ModelCatalogProduct extends Model
{
    ...
    public function getProduct($product_id)
    {
        ...
        (SELECT category_id FROM " . DB_PREFIX . "product_to_category AS p2c WHERE p2c.product_id = p.product_id),
        ...
        (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.type, p.sort_order FROM " . DB_PREFIX . "product p  INNER JOIN product_to_category as p2c ON p.product_id = p2c.product_id LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
    ...
    public function getProductid_by_model($sku)
    {
        $query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model = '" . $sku . "' LIMIT 1");
		$result = $query->row;
		if (!empty($result))
    ...
    public function getProducts_xsell($product_id)
    {
        $query = $this->db->query("SELECT xsell_id AS xsell_id FROM " . DB_PREFIX . "product_xsell WHERE product_id = '" . (int)$product_id . "'");
        $products = array();
        foreach ($query->rows as $result) {
            $products[] = $result['xsell_id'];
        }
        return $products;
    }

    public function getProducts_shortage($product_id, $shortage)
    {
        $query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE p2c.category_id IN (SELECT category_id FROM product_to_category WHERE product_id = '" . (int)$product_id . "') AND p.product_id != '" . (int)$product_id . "' LIMIT " . (int)$shortage);
        $products = array();
...
//                    }
                    ...
                    if (!empty($data['filter_description'])) {
                        $sql .= "LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(pd.description) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(p.model) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
                    } else {
...
//                    }
                    $word = trim(preg_replace('/\s\s+/', ' ', $data['filter_tag']));
                    $sql .= "LCASE(pt.tag) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' AND pt.language_id = '" . (int)$this->config->get('config_language_id') . "'";;
                }
    ...
    private $similar_price = 0;

    private function similar($a, $b)
    {
        if (abs($a['price'] - $this->similar_price) == abs($b['price'] - $this->similar_price)) {
            return 0;
        }
        return (abs($a['price'] - $this->similar_price) < abs($b['price'] - $this->similar_price)) ? -1 : 1;
    }
    ...
    public function getProductSimilar($product_id)
    {
        ...
        $query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE p2c.category_id IN (SELECT category_id FROM product_to_category WHERE product_id = '" . (int)$product_id . "') AND p.product_id != '" . (int)$product_id . "'");

        $product = $this->getProduct($product_id);
        $this->similar_price = $product['price'];

        foreach ($query->rows as $result) {
            $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
        }

        uasort($product_data, array($this, 'similar'));
        if (count($product_data) <= 10)
            $_product_data = $product_data;
        else {
            $_product_data = array();
            for ($i = 1; $i <= 10; $i++) {
                $_product_data[] = array_shift($product_data);
            }
        }

        shuffle($_product_data);

        if (count($_product_data) <= 4)
            $__product_data = $_product_data;
        else {
            $__product_data = array();
            for ($i = 1; $i <= 4; $i++) {
                $__product_data[] = array_shift($_product_data);
            }
        }

        return $__product_data;
    }
...
//                    }
    ...
    public function getProductColors($type)
    {
        $query = $this->db->query("SELECT p.product_id,p.color FROM product p
                                    WHERE p.`type`='" . $type . "' AND p.color != ''");
