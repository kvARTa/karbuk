        ...
        if (!empty($this->fields[$from]) && is_array($this->fields[$from])) {
            foreach ($this->fields[$from] as $id => $settings) {
                ...
                if (strpos($id, 'main_') === 0 && !array_key_exists($id, $this->display_fields[$from])) {
                    if ($from == 'checkout_customer' && $checkout_customer_use_selector && ($id == 'main_country_id' || $id == 'main_zone_id')) {
                        if (($id == 'main_country_id' && $checkout_customer_selector_type < 3) || ($id == 'main_zone_id' && $checkout_customer_selector_type > 1)) {
                            $tmp = $settings;
                            $tmp['selector'] = true;
                            $this->display_fields[$from][$id] = $tmp;
                        } 
                    } elseif ($from == 'checkout_address' && $checkout_address_use_selector && ($id == 'main_country_id' || $id == 'main_zone_id')) {
                        if (($id == 'main_country_id' && $checkout_address_selector_type < 3) || ($id == 'main_zone_id' && $checkout_address_selector_type > 1)) {
                            $tmp = $settings;
                            $tmp['selector'] = true;
                            $this->display_fields[$from][$id] = $tmp;
                        } 
